...
public class App {
    ...
    //wild magic, item, building or person,
    ...
    public static void populateMaps() {
        //populate  key array with {0.25, 0.5, 2.0, 4.0, 6.0, 10.0};
        ...
        //assign array items to themselves - species
        for (int i = 0; i < speciesKeys.length; i++) {
            speciesKeys[i] = i;
        }
        //assign array items to themselves - wild magic
        for (int i = 0; i < wildMagicKeys.length; i++) {
            wildMagicKeys[i] = i;
        }
        //assign array items to themselves - items
        for (int i = 0; i < itemKeys.length; i++) {
            itemKeys[i] = i;
        }
        //assign array items to themselves - people
        for (int i = 0; i < peopleKeys.length; i++) {
            peopleKeys[i] = i;
        }
        // assign array items to themselves - buildings
        ...
        //wild magic, items, people, buildings
        //populate wild magic map
    ...
    public static Object generateRandomEncounter() {
        ...
        //receive input for party size and save it as an int.
        var partySize = Integer.parseInt(scanner.nextLine());
        //turn the var into an int
        int partySizeInt = ((int) partySize); // this is marked as redundant, but if I change anything it stops working.
        ...
        //make input uppercase to avoid errors
        ...
        if (Objects.equals(inputEncounterType, "COMBAT") || Objects.equals(inputEncounterType, "C")) {
            ...
            if (Objects.equals(inputCombatEnviro, "WILDS")) {
                ...
                if (Objects.equals(wildEncounterBiome, "FOREST")) {
                    ...
                    //now that we have a challenge rating key, we can use it to get a monster to encounter
                    // but we need to dictate the number of monsters
                    // so it will need to be a loop
                    // if the challengeRatingKey is less than the party average, increase creature count by 1.
...
Repetitiveness in loops - extract that into a method

Hard coded strings that show up a couple of times - extract into CONSTANTS

Have it ask "Do you want to run it again?"
...
NOTES
  ...
  public Map<String, Integer> wordCount(String[] forestFights) {

  get a random number from a map
  List<A> keysAsArray = new ArrayList<A>(map.keySet());
   ...
   public static double getChallengeRating() {
        Random random = new Random();
        int index = random.nextInt(challengeRatingKeys.length);
        return challengeRatingKeys[index];
    }

    public static String getRandomWildMagic() {
        Random random = new Random();
        int index = random.nextInt(wildMagic.size()) + 1;  // Assuming wildMagic has keys 1 to n
        return wildMagic.get(index);
    }

    public static String getRandomItem() {
        Random random = new Random();
        int index = random.nextInt(items.size()) + 1;  // Assuming items have keys 1 to n
        return items.get(index);
    }

    public static String getRandomPerson() {
        Random random = new Random();
        int index = random.nextInt(people.size()) + 1;  // Assuming people have keys 1 to n
        return people.get(index);
    }

    public static String getRandomBuilding() {
        Random random = new Random();
        int index = random.nextInt(buildings.size()) + 1;  // Assuming buildings have keys 1 to n
        return buildings.get(index);
    }

    public static void generateRandomEncounter() {
        // Get random challenge rating
        double challengeRating = getChallengeRating();
        System.out.println("Challenge Rating: " + challengeRating);

        // Get random wild magic effect
        String wildMagicEffect = getRandomWildMagic();
        System.out.println("Wild Magic Effect: " + wildMagicEffect);
        ...
        // Get random building
        String building = getRandomBuilding();
